...
class InstitutionRepository extends BaseRepository
{
    ...
    private function countTotalRequestsOriginPerCountry($instance, $type, $initialYear, $finalYear)
    {
        ...
        if ($initialYear === $finalYear) {
            $query = $query->andWhere('YEAR(request.deletedAt) = :year')->setParameter('year', $initialYear);
        } elseif ($initialYear < $finalYear) {
            $query = $query->andWhere('YEAR(request.deletedAt) >= :initialYear')->setParameter('initialYear', $initialYear)
                            ->andWhere('YEAR(request.deletedAt) <= :finalYear')->setParameter('finalYear', $finalYear);
        }
