...
namespace Celsius\Celsius3MessageBundle\DocumentManager;
...
class ThreadManager extends BaseThreadManager
{
    ...
    public function getParticipantThreadsBySearchQueryBuilder(ParticipantInterface $participant, $search)
    {
        ...
        $qb = $this->dm->getRepository('CelsiusCelsius3Bundle:BaseUser')
                ->createQueryBuilder()
                ->hydrate(false)
                ->select('id');
        ...
        return $qb2->addOr($qb2->expr()->field('keywords')->equals(new \MongoRegex($regex)))
