...
namespace Celsius\Celsius3MessageBundle\Repository;
...
class ThreadRepository extends DocumentRepository
{
    ...
    public function applyExtraFilters(Builder $builder, Request $request, BaseUser $user)
    {
        ...
        } else if ($request->query->has('and') && $request->query->get('and') != '')
        {
            $and = new \DateTime($request->query->get('and'));
        ...
        {
            $qb = $this->dm->getRepository('CelsiusCelsius3MessageBundle:Message')
                    ->createQueryBuilder()
                    ->hydrate(false)
                    ->select('id');

            $messages = array_keys(
                    $qb->field('metadata')->elemMatch(
                                    $qb->expr()->field('isRead')->equals(false)
                                    ->field('participant.$id')->equals(new \MongoId($user->getId()))
                            )
                            ->getQuery()
                            ->execute()
                            ->toArray()
