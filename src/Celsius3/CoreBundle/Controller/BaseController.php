...
abstract class BaseController extends Controller
{
    ...
    protected function baseIndex($name, $filter_form = null)
    {
        ...
        return array('pagination' => $pagination,
                'filter_form' => (!is_null($filter_form)) ? $filter_form
                                ->createView() : $filter_form,);
    }
    ...
    protected function baseShow($name, $id)
    {
        ...
        if (!$document) {
            throw $this
                    ->createNotFoundException('Unable to find ' . $name . '.');
        }
    ...
    public function baseEdit($name, $id, $type, $route = null)
    {
        ...
        return array('document' => $document,
                ...
                'delete_form' => $deleteForm->createView(), 'route' => $route);
    }
    ...
    public function baseUpdate($name, $id, $type, $route)
    {
        ...
        if ($editForm->isValid()) {
            ...
            $this->get('session')->getFlashBag()
                    ->add('success',
                            'The ' . $name . ' was successfully edited.');

            return $this
                    ->redirect(
                            $this
                                    ->generateUrl($route . '_edit',
                                            array('id' => $id)));
        }
    ...
    public function baseDelete($name, $id, $route)
    {
        ...
        if ($form->isValid()) {
            ...
            $this->get('session')->getFlashBag()
                    ->add('success',
                            'The ' . $name . ' was successfully deleted.');
        }
    ...
    public function baseUnion($name, $ids)
    {
        ...
        $documents = $dm->getRepository('Celsius3CoreBundle:' . $name)
                ->createQueryBuilder()->field('id')->in($ids)->getQuery()
                ->execute();
            ...
            $updateInstance = true)
    {
        ...
        $main = $dm->getRepository('Celsius3CoreBundle:' . $name)
                ->find($main_id);
        ...
        $documents = $dm->getRepository('Celsius3CoreBundle:' . $name)
                ->createQueryBuilder()->field('id')->in($ids)->field('id')
                ->notEqual($main->getId())->getQuery()->execute();
        ...
        $this->get('celsius3_core.union_manager')
                ->union($name, $main, $documents, $updateInstance);
