...
class UserNotificationController extends BaseController
{
    ...
    {
        return parent::listQuery($name)->field('user.$id')
                ->equals(new \MongoId($this->getUser()->getId()));
    }
    ...
    public function getTemplate($code, $idiom)
    {
        $templates = $this->get('doctrine.odm.mongodb.document_manager')
                ->getRepository('Celsius3CoreBundle:Template')
                ->createQueryBuilder()->field('code')->equals($code)
                ->field('idiom')->equals($idiom)->getQuery()->execute()
                ->getNext();
        return $templates;
    }
    ...
    {
        $templates = $this->get('doctrine.odm.mongodb.document_manager')
                ->createQueryBuilder('Celsius3CoreBundle:Template')
                ->field('idiom')->equals($idiom)->getQuery()->execute();
        return $templates;
    }
    ...
    public function getNotificationToUser($cause, $user)
    {
        $notifications = $this->get('doctrine.odm.mongodb.document_manager')
                ->getRepository('Celsius3CoreBundle:Notification')
                ->createQueryBuilder()->field('user.$id')
                ->equals(new \MongoId($user->getId()))->field('cause')
                ->equals($cause)->field('viewed')->equals(false)->getQuery()
                ->execute();
    ...
    public function getMessageNotifiaction($cause)
    {
        ...
        if (count($notifications) > 0) {
            $templateNotification = $this
                    ->getTemplate($cause, $this->get('request')->get('_locale'));
            $notificationsMessagesArray = array();
            foreach ($notifications as $notification) {
                $env = new \Twig_Environment(new \Twig_Loader_String());
                $renderTemplate = $env
                        ->render($templateNotification->getText(),
                                array("notification" => $notification));
                array_push($notificationsMessagesArray,
    ...
    public function changeStateAction($id)
    {
        ...
        if (!$notification) {
            throw $this
                    ->createNotFoundException('Unable to find Notification.');
        }
        ...
        $this->get('session')->getFlashBag()
                ->add('success',
                        'The Notification was successfully '
                                . (($notification->getViewed()) ? 'enabled'
                                        : 'disabled'));
    ...
    public function indexAction()
    {
        ...
        $allTemplateNotification = $this
                ->getAllTemplate($this->get('request')->get('_locale'));
    ...
    public function getNotificationListAction()
    {
        ...
        return $this
                ->render(
                        'Celsius3CoreBundle:UserNotification:list.html.twig',
