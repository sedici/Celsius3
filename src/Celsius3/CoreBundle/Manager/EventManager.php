    ...
    public function getRealEventName($event, array $extraData, Instance $instance, Request $request)
    {
        switch ($event) {
            case self::EVENT__REQUEST:
                $event = ($extraData['provider'] instanceof Institution && $extraData['provider']->getCelsiusInstance() && !$request->getOrder()->hasRequest($extraData['provider']->getCelsiusInstance()) !== $instance->getId()) ? self::EVENT__MULTI_INSTANCE_REQUEST : self::EVENT__SINGLE_INSTANCE_REQUEST;
                break;
